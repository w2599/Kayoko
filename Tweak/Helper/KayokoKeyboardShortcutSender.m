...
- (instancetype)initPrivate {
    self = [super init];
    if (self) {
        _eventQueue = dispatch_queue_create("cn.zqbb.kayoko.queue.keyboard-shortcut",
...
- (void)sendCommandV {
    dispatch_async(_eventQueue, ^{
      NSLogDebug(@"[----] Kayoko: dispatching simulated Cmd+V HID shortcut");
...
- (void)dispatchKeyboardUsage:(uint32_t)usage isKeyDown:(BOOL)isKeyDown {
    if (!_eventSystemClient) {
        NSLog(@"[----] Kayoko: simulated shortcut skipped because IOHID event system client is unavailable");
        return;
    }
    ...
    if (!event) {
        NSLog(@"[----] Kayoko: simulated shortcut failed to create keyboard event usage=0x%x isKeyDown=%@", usage,
                   isKeyDown ? @"YES" : @"NO");
        return;
    }
